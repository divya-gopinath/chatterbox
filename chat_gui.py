...
class Application(tk.Frame):
    ...
    def initUI(self):
        ...
        self.microphone_img = ImageTk.PhotoImage(microphone)
        recording = Image.open("emoji_images/recording.png").resize((25, 25), Image.ANTIALIAS)
        self.recording_img = ImageTk.PhotoImage(recording)

        self.talk_btn = tk.Button(self, image=self.microphone_img, command=self.voice_to_text)
        self.talk_btn.grid(row=6, column=3)
        self.talk_btn.image = self.microphone_img
    ...
    def suggest(self):
        """
        Recommend emojis, to be triggered upon receiving a message from another person.
        """
    ...
    def voice_to_text(self):
        self.talk_btn.config(image=self.recording_img)
        self.talk_btn.image = self.recording_img
        root.update()
        speech = recognize_speech()
        if speech:
            self.entry.insert(0, speech)
        self.talk_btn.config(image=self.microphone_img)
        self.talk_btn.image = self.microphone_img
